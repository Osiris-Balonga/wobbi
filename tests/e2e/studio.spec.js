...
test('creates, animates, saves, exports and restores a mascot', async ({
  page,
  context,
}) => {
  const errors = [];
  ...
  await expect(preview.locator('[data-part="body"]')).toHaveAttribute(
    'fill',
    '#336699',
  );
  await page.getByRole('tab', { name: 'Grid', exact: true }).click();
  await expect(page.getByTestId('reaction-grid').getByRole('img')).toHaveCount(
    8,
  );
  ...
  expect(
    await page
      .locator('[data-testid="main-preview"] svg')
      .evaluate((el) => el.getAnimations({ subtree: true }).length),
  ).toBeGreaterThan(0);
  await page.getByRole('button', { name: 'Pause', exact: true }).click();
  await expect
    .poll(() =>
      preview.evaluate((el) => el.getAnimations({ subtree: true }).length),
    )
    .toBe(0);
  ...
  await page
    .getByLabel('Accessible label', { exact: true })
    .fill('Friendly cloud');
  await page
  ...
  await expect(
    page.getByRole('button', { name: 'View CloudBuddy.jsx' }),
  ).toBeVisible();
  const downloadPromise = page.waitForEvent('download');
  await page
    .getByRole('button', { name: 'Download source files', exact: true })
    .click();
  ...
  expect(Object.keys(files).sort()).toEqual([
    'src/buddies/CloudBuddy.jsx',
    'src/buddies/animations.js',
    'src/buddies/index.js',
    'src/buddies/styles.css',
  ]);
  const component = strFromU8(files['src/buddies/CloudBuddy.jsx']);
  expect(component).toContain('#336699');
  expect(component).toMatch(/^'use client'/);
  const dir = await mkdtemp(path.join(tmpdir(), 'wobbi-browser-export-'));
  try {
    for (const [name, data] of Object.entries(files))
      await writeFile(path.join(dir, path.basename(name)), data);
  ...
  await page.reload();
  await expect(
    page.getByRole('heading', { name: 'Cloud Buddy', exact: true }),
  ).toBeVisible();
  await expect(
    page.getByTestId('main-preview').getByRole('img'),
  ).toHaveAttribute('aria-label', 'Friendly cloud');
...
test('respects live reduced-motion changes and offscreen pause', async ({
  page,
}) => {
  ...
  expect(
    await svg.evaluate((el) => el.getAnimations({ subtree: true }).length),
  ).toBe(0);
  await page.emulateMedia({ reducedMotion: 'no-preference' });
  await expect
    .poll(() =>
      svg.evaluate((el) => el.getAnimations({ subtree: true }).length),
    )
    .toBeGreaterThan(0);
...
test('keyboard navigation, docs dialog and real contrast checks', async ({
  page,
}) => {
  await page.goto('/');
...
test('small screen keeps customization, preview and export reachable', async ({
  page,
}) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole('heading', { name: 'Mochi', exact: true }),
  ).toBeVisible();
  await page
    .getByRole('navigation', { name: 'Workspace panels' })
