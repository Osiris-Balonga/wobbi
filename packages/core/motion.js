...
export function mountMotion(element, motion, preferences, playing) {
  ...
  const update=()=>{
    ...
    for(const step of animationPlan(motion,reduced)) {
      ...
      if(target?.animate) animations.push(target.animate(step.keyframes,step.options));
    }
  ...
  let observer;
  if(preferences.pauseOffscreen && typeof IntersectionObserver!=='undefined') {
    observer=new IntersectionObserver(entries=>{visible=entries[0].isIntersecting;animations.forEach(a=>visible?a.play():a.pause());});
    observer.observe(element);
  }
  return ()=>{cancel();media.removeEventListener('change',update);observer?.disconnect();};
}
